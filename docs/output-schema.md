...
统一错误格式（自 2026-07-07 起，8 个 skill 通过 `shared/errors.py` 输出）：
...
`step` 字段是发起本次失败请求的 skill 名称（`voice-clone` / `voice-infer` / `voice-separate` / `speaker-classify` / `timbre-design` / `subtitle-erase` / `videots` / `video-compose`），便于 `/lychee-workflow` 组合链路定位失败点。`hint` 仅在异常类型可恢复时出现（如 API key 缺失、文件权限、网络中断）。
...
- `task_id` vs `taskId`：`videots-lychee` 和 `video-compose-lychee` 都通过 `task_id_from()` 辅助函数同时读取两种后端字段名（`result.get("task_id") or result.get("taskId")`），对外 stdout 一律输出 `task_id`。**该差异已在代码层统一，不需后续改动。**
...
- `status=success` vs `status=completed`：`speaker-classify`、`voice-separate`、`subtitle-erase` 成功终态是 `success`；`videots`、`video-compose` 成功终态是 `completed`。`shared/poll_status.py` 的 `success_states` 参数对应这种差异。
- 错误 JSON 字段（自 2026-07-07）：8 个 skill 用 `shared/errors.py` 输出 `step` + `hint`，详见 "统一错误格式"。`asr-lychee` 仍输出纯文本。
