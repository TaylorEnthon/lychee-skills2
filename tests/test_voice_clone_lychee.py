"""voice-clone-lychee skill 的 smoke 测试。"""
import pytest
...
def test_skill_md_has_yaml_frontmatter(skill_dir):
    assert_skill_frontmatter(skill_dir, "voice-clone-lychee")


def test_script_help_runs(skill_dir):
    assert_script_help(skill_dir, "clone.py", ["--file", "--carry-back"])
