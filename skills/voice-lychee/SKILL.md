...
```bash
# 全命中 → voices 模式
python scripts/synthesize.py --voice-names Vivi --voice-names 小何 --text "你好" --output ./out.mp3
# stdout: {"mode": "voices", "polish": "auto", ...}

# 部分缺失 → 整体降级到 text 模式（让 AI 自动选音色）
python scripts/synthesize.py --voice-names Vivi --voice-names 温柔女声 --text "你好" --output ./out.mp3
# stdout: {"mode": "text", "polish": "auto",
#          "fallback_reason": "音色「温柔女声」不在公共音色池（Vivi）；整体降级为纯文本模式（让 AI 自动选音色）。", ...}

# 全缺失 → 整体降级到 text
python scripts/synthesize.py --voice-names 温柔男声 --voice-names 未知音色 --text "你好" --output ./out.mp3
# stdout: {"mode": "text",
#          "fallback_reason": "所有音色名都不在公共音色池（温柔男声、未知音色）...", ...}
```
...
python scripts/synthesize.py --text "沈清晏：求陛下明察！贵妃：你胡说的。" --output ./out.mp3
# stdout: {"polish": "auto", ...}  # agent 自己决定

# 强制不润色（用用户原文本）
python scripts/synthesize.py --text "沈清晏：求陛下明察！贵妃：你胡说的。" --no-polish --output ./out.mp3
# stdout: {"polish": "skipped", ...}

# 强制润色（即使 agent 判断不需要）
python scripts/synthesize.py --text "沈清晏：求陛下明察！" --polish --output ./out.mp3
# stdout: {"polish": "forced", ...}
```

详细润色规则见 `references/script-polishing-guide.md`。
...
| --- | --- |
| `--text` | 必填，最多 3000 字符 |
| `--voice-ids` | 公共音色 ID，1–3 个；可重复传或逗号分隔。**与 `--voice-names` 互斥**。 |
| `--voice-names` | 公共音色 name（自动查表转 id）；任何一个 name 找不到时整体降级为纯文本模式（让 AI 自动选音色）。可重复或用逗号分隔。**与 `--voice-ids` 互斥**。 |
...
| `--polish` | 强制润色（agent 跳过自动判断，直接走润色路径）。与 `--no-polish` 互斥。 |
| `--no-polish` | 禁用润色（agent 不要自动润色，用用户原文本）。与 `--polish` 互斥。 |
| `--output` | 本地音频路径 |
| `--timeout` | 请求及下载超时秒数，默认 120 |

## stdout 字段

`scripts/synthesize.py` 成功时 stdout JSON 字段：
...
| `mode` | string | 实际走的模式：`text` / `voices` / `image` / `audio_url` |
| `polish` | string | 润色策略：`auto`（agent 自动判断）/ `skipped`（用户禁用）/ `forced`（用户强制） |
| `fallback_reason` | string? | 音色降级说明（仅在 `--voice-names` 找不到任一 name 时出现） |

## Red flags

- `--image`、`--audio-urls`、`--voice-ids` 三选一互斥。
- `--voice-ids` 和 `--voice-names` 互斥（不能同时传）。
- `--polish` 和 `--no-polish` 互斥（不能同时传）。
...
- `--voice-names` 解析依赖 `/open/voice/lychee-voice/list` 返回——若列表接口异常，synthesize 会报"查询音色池失败"。
